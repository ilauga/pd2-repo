...
# Setting NASA url  & key
nasa_api_key = "dQ9TVsaiOTDMRE6q1zA3i1UaiEChoW2jpWUzQ9HK"
...
# Prints data received from NASA
print("Response status code: " + str(r.status_code))
print("Response headers: " + str(r.headers))
print("Response content: " + str(r.text))

# Checks if got successful response(code 200) from NASA, if all good - starts to process data
if r.status_code == 200:

	# Received data are parsed  as a json structure
	json_data = json.loads(r.text)

	# Two arrays are created to save data for each (safe or hazardous) asteroids
	ast_safe = []
	ast_hazardous = []

	# Checks if element exists ('element_count' is based on response structure from NASA and means expencted data is  present)
	if 'element_count' in json_data:
		# Saves asteroid count for today and prints it
		ast_count = int(json_data['element_count'])
		print("Asteroid count today: " + str(ast_count))
		# If asteroid count isn't zero,  enter IF
		if ast_count > 0:
			# Goes through each asteroid for today
			for val in json_data['near_earth_objects'][request_date]:
				# Cheks if required data is available
				if 'name' and 'nasa_jpl_url' and 'estimated_diameter' and 'is_potentially_hazardous_asteroid' and 'close_approach_data' in val:
					# Gets name of asteroid
					tmp_ast_name = val['name']
					#Gets  url of asteroid description
					tmp_ast_nasa_jpl_url = val['nasa_jpl_url']
					# Gets diameter values
					if 'kilometers' in val['estimated_diameter']:
						...
						tmp_ast_diam_min = -1
						tmp_ast_diam_max = -1
					# Gets answer if  asteroid is potencially hazardous
					tmp_ast_hazardous = val['is_potentially_hazardous_asteroid']

					# Checks if  there's data for close approch
					if len(val['close_approach_data']) > 0:
						# Goes through data for close approch as date, velocity, missing distance
					...
					# Prints data for todays asteroids
					print("------------------------------------------------------- >>")
					...
					# Adding asteroid data to the corresponding array (if potentially hazardous)
		...
		# If no asteroids for today, print  info
		else:
			print("No asteroids are going to hit earth today")
	# Print overall statement from processed data via IF statement - number of hazardous and safe asteroids
	print("Hazardous asteorids: " + str(len(ast_hazardous)) + " | Safe asteroids: " + str(len(ast_safe)))

	# If any potenciallu hazardous asteroids, enter IF
	if len(ast_hazardous) > 0:

		# Sorting asteroids by time they will hit the Earth, and prints (name, time, url for more info)
		...
		# Sorting asteroids(array) by passing distance and prints data for closest
